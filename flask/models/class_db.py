...
def hw_update_class(class_):
    session = SessionLocal()
    # print('\n\n---------------------------------hw_update_class_------------------------------------\n\n')
   
    curr_class_ = session.query(Class).filter(Class.class_name == class_['class_name']).one_or_none()
   
    hw_remove_class(curr_class_.class_name)
    hw_add_class(class_)
    session.commit()
    session.close()

def hw_get_students_by_class():
    session = SessionLocal()
...
def hw_add_student_to_class(student, class_name):
    session = SessionLocal()
